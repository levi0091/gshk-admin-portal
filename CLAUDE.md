...
> **THE COPY IS `renewal@getstarted.hk`, AND THE CASE WORKER IS NO LONGER CC'd** (Levi, 2026-09-08). This **reverses** *"the case worker is CC'd"* (2026-08-30), under which `POST /cases/{id}/verification/send` put `user["email"]` — whoever pressed Send — on the CC line of a letter about a client's statutory return. Two things were wrong with that: the client saw an individual staff member's personal address on a formal filing notice, and GSHK's record of what it told that client lived in one person's mailbox, leaving with them when they were on leave or off the account. The copy is now `email_service.CLIENT_CC`, a **module constant** exactly like `TEST_RECIPIENTS` and for the same reason — which address hears about a client's return is not a per-deployment setting somebody can repoint by editing a Railway variable.
>
> **It is on EVERY message, where the case worker's copy was on the first only.** That rule existed so three directors would not mean three identical mails in one inbox, but these messages are *not* identical — each carries its own approval link and greeting — and a shared mailbox holding the first of three would misrepresent a partial send as a complete one. `renewal@` is a record, so it gets the whole record.
>
> **`reply_to` is still the case worker, deliberately — but it is now the SAFETY NET, not the asked-for path.** Since 2026-09-08 the letter tells the client that replies are **not monitored** and to write to `renewal@getstarted.hk`; it is nonetheless sent from `no-reply@getstarted.hk`, and a client who replies regardless must reach a human who knows the case rather than a black hole. The copy going to the team while a stray answer goes to a person is the intended split. Read `user.get("email")`, never `user["email"]` — identities are cached for 30s and one written before the key existed must not break a send.
>
> **The screen names the address, and a test stops the two drifting.** The note on Client Verification promises `renewal@getstarted.hk` by name *before* the send exists, so it cannot read the value off a response — it repeats the constant in `StageClientVerification.jsx`. `test_client_cc_matches_the_screen` reads that JSX and fails if it ever stops matching `email_service.CLIENT_CC`.
>
> **A CC is a recipient, so the non-production lock binds it too**: `_apply_test_cc_lock` **drops** it outside prod rather than redirecting. That is now load-bearing in a way it was not before — `renewal@getstarted.hk` is a **real GSHK mailbox and deliberately NOT one of the four `TEST_RECIPIENTS`**, so a test deployment must never reach it. (The old reasoning, "the four are already on `to` and copying them again puts one mailbox on both lines", described the case worker and no longer applies.) The guard before the HTTP call checks `to` **and** `cc`. The audit row records `cc` and `intended_cc` separately, so a dropped copy is never logged as a delivered one.

> **Resend answers 200 for an address that cannot exist, so the send probes DNS first** (Levi, 2026-09-08). A verification mailed to a made-up domain reported **total success** on the Client Verification screen: the address was well-formed so the syntax gate passed it, Resend accepted it and bounced it out of band minutes later, and `EmailError` — which only fires on HTTP ≥ 400 — had nothing to catch. `email_service.undeliverable_reason()` now asks whether the domain can receive mail at all (MX, or the A/AAAA fallback RFC 5321 §5 allows) and the router drops the ones that cannot, reporting them in the **same `failed` list** as a malformed chip and a Resend rejection — the operator's question is "who did not get it", and the answer must not depend on which of three ways an address failed.
>
> **It runs after both recipient branches, on purpose.** The syntax gate only ever ran on addresses an *operator typed*, which left the commoner case unchecked: a director address out of Viewpoint on a since-lapsed domain went to Resend unexamined.
>
...
>
> **A REDIRECTED SEND IS NEVER REPORTED AS DELIVERED, and this is the subtlest trap on the whole path.** Outside production the recipient lock substitutes `TEST_RECIPIENTS` *inside* `send()`, so Resend's record for that message describes the four internal mailboxes — **not** the address the operator chose. Printing Resend's `delivered` beside the intended address told Levi on 2026-09-08 that `levi214839824@zenexflow.com`, an address he knew did not exist, had been **Delivered**. Resend was not wrong; the pairing was. Verified against that exact send: both messages' `to` came back as the four test recipients.
>
> `deliveries` therefore records **`delivered_to` and `redirected`** alongside the intended address — the same reason the audit row carries both `to` and `intended_to`. A redirected entry gets status **`redirected`**, reads *"Not sent (test)"*, is **not green**, and **Resend is not asked about it at all**, because its answer could only describe somebody else's mailbox.
>
> **So bounce detection cannot be exercised on DEV**, and the screen now says so rather than implying a pass. The lock replaces every address with four real mailboxes that deliver; only production can confirm delivery to a client address.

> **The splash opens on the CLICK, not on the response.** The send fills a nine-page AcroForm, issues a token per director and makes one Resend call each, so there is a real multi-second gap — and until 2026-09-08 nothing appeared during it, so the button read as dead (Levi: *"there seems to be a lag between when i click on the send button and the popup appearing"*). It opens seeded from the chips, which are exactly who is about to be written to. **It does not poll until the send returns** (`phase`): there is no audit row yet, and asking early would read the PREVIOUS send on that case and report a fresh one as already settled. The 60s clock starts when polling does, not when the modal opened. A **refused** send takes the splash down, or the page banner would be drawn behind the overlay.
>
...
>
> **Its wording is `docs/Auto email - NAR1 Review_v2.pdf`, verbatim** (Levi 2026-09-08). This **supersedes `docs/Confirmation NAR1 Notice.pdf`**, the hand-sent letter the message carried until then, and four of the differences are decisions rather than restyling: it opens **"Dear Client"** and not a director's name; it is signed **"Kind regards, Get Started HK Limited"** with **no case worker and no "Account Manager"** (`verification_email` therefore takes **no `recipient_name` and no `sender_name`** — they were removed, not ignored); it states that **replies are not monitored** and points changes at **`renewal@getstarted.hk`** (`email_service.RENEWAL_MAILBOX`, which `routers/public_approval.py` imports rather than re-typing); and the **HK$1,000** fee is for changes requested **after filing**, which is a later event than the "any amendments later" the old letter charged for. The page references (**Page 2** Share Capital, **Page 5** Director's Details, **Schedule 1** Shareholder's Details, **Continuation Sheet C** *if applicable*) are **hardcoded and correct**, because CR's form is static and a section keeps its page whether or not it has content. They are only true while the renderer emits all nine pages; if it ever goes back to dropping empty ones, this letter misdirects every client.
>
> **THE "NO LINK AT ALL" RULE IS REVERSED** (Levi 2026-09-01). It read: *"the client confirms by replying, and a mail about someone's company filings that contains a link is the exact shape of the phishing it would train them to trust."* The reasoning still stands; what changed is that replying by hand was the **only** way to answer, so every approval waited on a staff member reading a mailbox and a client who never replied left a case parked with nothing recorded. The message now carries a **Confirm NAR1** button (`routers/public_approval.py`). What answers the original objection, rather than going around it: the link's `GET` **mutates nothing** and says so, so the mail-security gateway that fetches every link cannot approve anything; and the page asks for **no credential, no password, no payment detail**, so there is nothing on it worth phishing for. `approval_url` is `None` when the deployment cannot build one, and the letter then asks for a **reply** instead — the only place its wording departs from the sample, because "click Confirm below" beside no button is worse than a sentence the sample does not contain, and the automated/"replies not monitored" line is **withheld** on that path for the same reason.
>
> **The reply path is no longer what the client is TOLD to use** (Levi 2026-09-08), which reverses the previous line here. `reply_to` is still set (see below) so a client who replies anyway reaches a human, and staff still record a relayed answer through `POST /cases/{id}/verification/response` — but the letter names `renewal@getstarted.hk`, and a screen or a page that tells an operator or a client to "reply to the email" is now wrong.
>
> **The client's confirmation page shows the approved Warning panel** (`public_approval._ask`): *"By clicking **Confirm & File**, you confirm that you have reviewed the draft and that the information is correct. The NAR1 will then be arranged for submission without further confirmation."*, and the button reads **Confirm & File**. The mock draws it as a **modal**; it is rendered **inline, visible on arrival**, because this route runs **no script at all** (`test_the_page_runs_no_script_at_all`) and a modal needs some. Inline says the same words at the same moment, before anything can be pressed. There is **consequently no Cancel** — in the mock it dismisses a dialog, and here there is no dialog to dismiss.
>
> **The client's deadline is MANDATORY and the case worker picks it** (Levi 2026-09-07). `POST /cases/{id}/verification/send` requires `respond_by` (a calendar date, refused if it is in the past) and the Client Verification screen will not arm Send without one. It used to be `sent + 14 days`, computed inside the token issuer and chosen by nobody — and a fortnight is not a business rule: some clients are chased inside a week, and a return prepared months before its filing window should not have its link die long before anyone intends to file. **It is one value doing three jobs on purpose** — the date the letter prints, the moment the Confirm link stops working, and the moment `jobs.auto_approve_nar1` reads the client's silence as consent — passed as one argument (`nar1_approvals.issue(expires_at=...)`) so the message cannot promise a date the job does not honour. It expires at **23:59:59 Hong Kong** on the chosen day, not at midnight UTC, which would kill the link at 08:00 on the morning the client was given. `APPROVAL_WINDOW_DAYS` survives only for callers with no operator to ask.
>
> **A malformed address no longer refuses the whole send** (Levi 2026-09-07). It used to raise 422, so one typo'd chip left a three-director board unmailed. Since the send is one message per director there was never a reason the other two could not go: the bad address is reported by name **with its reason** in `failed` (alongside anything Resend rejects), the valid ones are sent, and the screen names **both** lists — who has it and who does not. A list with no valid address in it is still a 422, and names them. The check itself is unchanged: free text is never handed to Resend.

> **One message per recipient, not one message with several recipients.** This also reverses a stated decision (*"a board of three directors is one message with three recipients ... one Resend failure cannot leave two directors informed and the third not"*). A shared link in a shared message would let any director approve in another director's name, which is a misattribution in a statutory record. The cost — partial failure — is handled by naming exactly which addresses failed (`failed_to`) rather than pretending the send was atomic. `renewal@getstarted.hk` is CC'd on **every** message and `reply_to` is on every one too — the case worker's first-message-only copy was removed on 2026-09-08, see above.
...
> **The case workflow stepper has SIX stages, not v11's five** (Levi 2026-09-16). Stage 6, **CR Status**, is the only one that is not GSHK's: the first five end at "we handed it over" and this one begins there. `.stepper` is `repeat(6, 1fr)` at every width, with a tighter medallion below 720px.
>
> **Its medallion is coloured by CR's answer, not by our progress** (`workflow.stageTone`, the `.step.tone-*` rules). Every other step says where WE got to; this one says what the register decided, so the verdict is readable off the progress bar without opening the stage. It is the only place data drives the chrome, and `cr_registered` gives the tick rather than a tone — a green medallion without a tick would make the finished case the one state with no tick anywhere.
>
> **The badge family is `bc-*`, a third one.** `bw-*` is where the case is with GSHK, `bf-*` is where the document got to in our own CR chain, `bc-*` is what CR's register says. `--peridot-ink` (#7A5E00) was added to the palette for it: `--peridot` is a warning FILL and is illegible as type on its own tint, and shipping an unreadable badge to keep the token list at its original length would be the wrong economy.
...
|--------|-----------|----------------------------|
| Companies | `companies` | `read`, `write` — **including the company's documents** |
| Persons | `persons` | `read`, `write` — **including the person's documents** |
...
> **`documents` IS NOT A MODULE (migration 040, Levi 2026-09-07).** It was one, with `read`/`write`/`delete` of its own, and being separate meant it could disagree with the record it describes in both directions: a role granted Companies (edit) could not upload the certificate of incorporation for a company it was trusted to edit the CR number of, and a role holding `documents:delete` and no `companies` grant at all could remove a company's papers without ever being able to open the company. A document is filed **against** a person, a company or a case and now takes that record's grant — `read` downloads it, `write` uploads and removes it. **There is no successor to `documents:delete`:** neither owner module has a delete level, and removing a document is a write on the owning record. This is the permission half of the decision migration 037 already made for the audit trail's `module` column.
>
> The routes in `routers/documents.py` are keyed by document id alone, so they cannot use a static `require_permission(...)` — `services/document_permissions.require_document_permission` reads the owner first and **fails closed** on a document it cannot attribute. That module imports `middleware.auth` **as a module, never its names**: binding `_resolve_user` by value here made `patch("middleware.auth._resolve_user")` a no-op and sent a unit test at live Supabase.
...
| `TPSI_DOC_STATUS_CHECKED` | TPSI | `docStatusEnquiry` asked about a filed return — **every** check (migration 043) |
| `NAR1_CR_STATUS_CHANGED` | NAR1 | CR's answer about a filed return MOVED (migration 043) |
...
> **THE WORKFLOW ENDS IN CR'S ANSWER, NOT IN A WORD OF OURS** (Levi 2026-09-16, migration 043). `completed` is **gone from the vocabulary** — removed, not aliased. It said nothing CR had agreed to: a case went green the instant `submitFormNar1` returned a receipt, while CR had only RECEIVED the return and could still refuse it. The same defect left the Confirmation step permanently at IN PROGRESS beneath a CR receipt, because `stageDone(5)` waited on `tpsi_filings.stage = 'registered'` — a value migration 018 defined ("CR confirmed the filing via docStatusEnquiry") and **nothing ever wrote**.
>
> In its place are six CR codes from `services/tpsi/doc_status.py`, and a **sixth workflow stage, CR Status**. `cr_not_checked` is the honest state of every filed return until something asks: filed, and grey.
>
> **CR's `documentStatus` vocabulary is OPEN, and that is a measurement.** `TPSI API Interface v1.0.14.docx` §6.5.4 declares it `String(20)` with the remark "Document Status" and **enumerates no values**; the User Guideline does not contain the word "status". Three have been seen: `Registered`, `Pending`, and **`Lodged`, which is what CR actually said about all eight filed DEV cases the first time the poll worked** (2026-09-16) and which no guessed-at phrase had covered — it was found an hour earlier in the response example embedded in §6.5.5 of that same docx. It normalises to `cr_pending`: lodged is the receipt, registered is the decision. So CR's exact words are stored and shown verbatim, a *treatment* is derived from them by one fuzzy normaliser, and anything unanticipated lands on `cr_unknown` — grey, with CR's own wording on the badge — rather than on a wrong colour. **`cr_not_checked` and `cr_pending` are separate on purpose**: "we have not asked" and "CR says pending" are different facts.

> **THE URL SEGMENT AND THE OPERATION ARE DIFFERENT WORDS — `docStatusEnquiry` vs `cr:enquireDocStatus`** (2026-09-16). The path is `/tpsi/docStatusEnquiry` (§6.5.1) and the `soap:Body` element is `cr:enquireDocStatus` wrapping `cr:criteria` (§6.5.3) — exactly the split `soap.OPERATIONS` already records for e-Drive, and the response tag `enquireDocStatusResponse` said so all along. Sending the path's word as the operation authenticated fine and came back `Message part {…}docStatusEnquiry was not recognized. (Does it exist in service WSDL?)` for **every filed case in the book**, and on screen that read as *"Could not reach the server"* — see the 422 note below. **The examples are EMBEDDED WORD DOCUMENTS inside the docx** (`word/embeddings/*.docx`), not text in the body, which is why §6.5.3 looks empty when the docx is read as paragraphs; unzip it. The reply nests `result > documentList > document` (one row per **document**, not per `result`) and spells the BR number `brno` where the request spells it `brNo`.

> **A CR FAULT WITH NO FAULT BEANS IS A 422, NOT A 502.** `raise_for_fault` raises a bare `TpsiError` when CR's SOAP *stack* refuses rather than its business rules — and that is the sentence naming the bug. On 502 it never arrives: Cloudflare replaces the body with its own page, which carries no CORS header, so `fetch` **rejects** and `api.js` prints its offline message for a server that answered. 502 is kept for `TpsiAuthError`, where the frontend's static hint carries the meaning without needing a body. A `TpsiValidationError` out of **`docStatusEnquiry`** is not a refusal of anything either: that call submits no return, so the only thing CR can fault is the criteria we sent (`Case no does not exist.` for a mistyped `manual_receipt->>'caseNo'`). `nar1_cr_status.refresh` turns it into a **skip carrying CR's words**, so nothing is written and one bad case number cannot abandon a 400-case run.

> **AN EMPTY `documentList` MEANS CR KNOWS THE CASE AND HAS LISTED NOTHING — IT IS NOT A BAD CASE NUMBER.** Measured against both CR environments on 2026-09-17: a case number CR does not hold comes back as a **fault** (`Case no does not exist.` — checked with `999999999` and `wddw` on production *and* test), never as an empty list. So an empty list is positive evidence the key was right. Two real PROD returns filed at 16:21 and 17:29 HK were still unlisted at 00:31 that night — by case number, by document reference **and** by BR number over a twelve-month window — while every DEV case CR has ever answered about was at least **two days** old when first asked. Treat "filed today, nothing listed" as normal and let the schedule pick it up.
>
> **A SKIP STILL RECORDS `cr_status_checked_at`.** We asked; CR answered; only the answer was "nothing". Leaving it NULL was wrong three ways at once, all visible on PROD after ~20 real checks: the stage read "Not yet checked with CR" about work that had happened, `poll_cr_status` orders `NULLS FIRST` so those cases headed **every** run forever, and nobody could tell "nobody has asked" from "we ask every 15 minutes and CR has not listed it".

> **`trg_set_updated_at` OVERRIDES YOUR PATCH — IT IS `NEW.updated_at = now()`, UNCONDITIONALLY** (migration 044). `nar1_cr_status.refresh` sets `updated_at` only when CR's answer actually moved, with a comment explaining that a heartbeat touching it "would reset every filed case in the book"; the trigger had been silently doing exactly that on every write since 003. Survivable nightly, not at **every 15 minutes** — the dashboard's sortable Last Updated column would rank filed cases by "is this still being polled", the same answer for all of them. 044 adds `WHEN (NEW.cr_status_checked_at IS NOT DISTINCT FROM OLD.cr_status_checked_at)` to the trigger **on `nar1_cases` only**; `set_updated_at()` itself is shared by ~30 tables (003, 016, 020) and must not be edited to fix one. This works because `cr_status_checked_at` has exactly one writer, `nar1_cr_status._store`.
>
> **`now()` IS THE TRANSACTION TIMESTAMP, so a DB test comparing it against itself proves nothing.** The first draft of `tests/test_migration_044.py` passed whether or not the trigger fired, because insert and update shared one transaction and therefore one `now()`. Seed a date years back — and seed `cr_status_checked_at` too, or the heartbeat's `now()` equals what is already stored, `IS NOT DISTINCT FROM` reads it as unchanged, and the trigger fires after all.
>
> **The four columns live on `nar1_cases`, not `tpsi_filings`** — the one deliberate exception to D-6's split. A return signed on paper and filed off-portal has a CR case number (`manual_receipt->>'caseNo'`) and **no submitted filing row**; writing "CR registered it" onto a row whose own stage says `validated` would be a record that contradicts itself. `services/nar1_cr_status.py` is their only writer, and it stores the **normalised code**, never re-derived in SQL — the view reads a stored fact exactly as it reads `filing_stage`.
>
> **A CR rejection is NOT `submission_failed`.** That stage means `submitFormNar1` itself refused and **nothing was charged**. A rejection after filing means the opposite — delivered, charged, then refused — so the stage stays `submitted` and only `cr_doc_status_code` moves. `submitted → registered` IS promoted, on the e-Sign path only.
...
**The Date beside the signature is the day CR RECEIVED the return, and is EMPTY until it has** (Levi 2026-09-22). An anniversary of 26 October and a filing on 1 November prints **01/11/2026**. `render(submitted_on=...)` — renamed from `signed_on`, so the old name cannot be passed by muscle memory — and every one of the four callers resolves it through **`nar1_cases.filed_on(filing, case)`**, never by reading a column itself.

> **This reverses "an absent value means today in Hong Kong"** (2026-09-04), which was itself a reversal of "blank when the caller supplies nothing". Dating an unfiled copy *today* fixed an empty box by putting a wrong date in it, two ways at once: a director approving on 28 October saw 28 October on a return CR would not receive until 1 November, and one return downloaded on two days showed two different dates. What CR prints there is the day CR got the form; until then there is no such day, and a blank box says so where a plausible date does not. **`signature_date()` returns `""` for anything it cannot read** — a fallback date is indistinguishable on the page from a real one.
>
> **Not `signed_at`.** Signing and submitting are separate CR calls: a return signed at 23:55 and submitted at 00:10 was filed the *next* day, and one signed but never submitted was never filed at all. **Both roads to the register count** — the e-Sign path writes `tpsi_filings.submitted_at`, while the off-portal path never touches the filing row (it is signed on paper and lodged by hand) and records the day on `nar1_cases.manual_submitted_at`. Reading only the filing row re-dated every manually filed return to whatever day somebody downloaded it. `filed_on` is **pure** — both rows are passed in — because a query hidden in it would run inside the renderers' refusal paths and inside every router test that patches the renderer but not Supabase.
>
> **Hong Kong, not UTC** — Railway and Supabase both run UTC, and a return filed at 01:00 in the office is 17:00 the previous day there. Only a *timestamp* is converted; a plain date is taken verbatim.
>
> The date is **not in CR's XML** either way — CR hands none back — so this is the portal's rendered copy only, and `compare.py` composes both sides undated so a filing date can never be reported as a change the client did not approve.

**A box CR fills that the renderer used to leave empty** (Levi 2026-09-04), now filled by `fill.py` rather than by a caller that has to remember:
...
**"Date to which this Return is Made Up" is CR's to write, and before CR has, it is the incorporation anniversary** (Levi 2026-09-18). `dateReturnMadeUp` is *"filled in by system after Web-Form validation"*: CR's validate example omits it and its submit example carries it. Do **not** add it to the mapper, since `request_xml` follows CR's validate example. When Client Verification became stage 1, the client started getting `request_xml`, and section 4, the Schedule 1 header and every continuation-sheet header went out **blank**. `fill.made_up_date()` now prints CR's value when there is one, and otherwise the incorporation anniversary in the return's own `yearAnnualReturn`, computed by `fees.return_date_for`, the same function the late fee and the early-filing gate use. On DEV it matched CR's own date on 41 of 41 validated filings. **Every `render()` caller must pass `incorporated_on=entity.get("incorporation_date")`**, which is not in the XML. `nar1_cases.entity_for` selects that column, and a test pins it, because route tests patch `entity_for` wholesale. With no incorporation date the boxes stay empty rather than invented.

**The shipped return is FLAT — it carries no form fields at all** (Levi 2026-09-14). `appearance.bake()` draws every value into the page and then **removes** every widget and the AcroForm. It used to only set each widget's Hidden flag, leaving `/V`, the template's `/Q 1` and a `/PMingLiU 12` appearance stream in place; desktop viewers honour Hidden, **iOS Quick Look does not**, and on a phone every value printed twice — ours, and a centred 12pt copy on top. **Never bring a field back into anything a client or CR sees.** `bake()` refuses (`AppearanceError`) a widget holding a value it cannot draw, rather than delete it silently. Tests that need a value's box or `/V` use `fill.render_fields()` — the same return before baking, which must never be sent to anybody. The removed widgets are swept by re-cloning from the root: pypdf's `compress_identical_objects(remove_unreferenced=True)` is one pass and kept all 222 of them.
...
> **A NAR1 cannot be submitted before its return date** (Levi 2026-09-07). An annual return reports on the year ending at the company's return date — the anniversary of incorporation **in the return's own year** — so `filings.submit` refuses with `BeforeReturnDate` (a `SubmitGateError`, hence a 409 and a `TPSI_SUBMISSION_FAILED` row) **before any CR traffic at all, including the free balance read**, and before the drift check: "the data moved" is a confusing answer to "you are eleven months early". `preview` reports the same thing as `too_early` + `return_date` so the Submission screen withholds the button and prints the reason instead of arming an irreversible-looking control whose one outcome is a 409.
>
> `fees.annual_return_fee` already treated a future return date as uninterpretable, but that only downgraded the quote to the HK$3,480 ceiling — **the submit went ahead anyway**, so a case opened a year early could be validated, signed and filed, and the first thing to notice would have been CR, after the charge.
>
> **It fails OPEN when the date cannot be computed**, which is the one gate in that file that does. A missing incorporation date or an unreadable return year is not evidence that a return is early, refusing on it would block a filing due today over a blank field, and the fee already degrades to the ceiling for exactly those filings. `filings.return_date_of` returns `None` there, and `too_early` stays `False` while `return_date` is `None` — the screen must not read one as the other.
