...
>
> **CR's `documentStatus` vocabulary is OPEN, and that is a measurement.** `TPSI API Interface v1.0.14.docx` §6.5.4 declares it `String(20)` with the remark "Document Status" and **enumerates no values**; the User Guideline does not contain the word "status". The only two CR has ever sent this codebase are `Registered` and `Pending`. So CR's exact words are stored and shown verbatim, a *treatment* is derived from them by one fuzzy normaliser, and anything unanticipated lands on `cr_unknown` — grey, with CR's own wording on the badge — rather than on a wrong colour. **`cr_not_checked` and `cr_pending` are separate on purpose**: "we have not asked" and "CR says pending" are different facts.
>
