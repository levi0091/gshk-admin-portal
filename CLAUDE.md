...
| Email | **Resend** | Transactional emails — client verification, notifications. Sends from **`no-reply@getstarted.hk`** via the client's Resend account (`getstarted.hk` is a domain GSHK controls; verify SPF/DKIM in Resend). Levi 2026-08-16. **`RESEND_API_KEY` in the local `.env` is still the literal placeholder `your-resend-api-key`** — Resend answers "API key is invalid" because no key was ever put there, not because one was revoked. Until a real key is supplied, `EMAIL_TRANSPORT=console` is the only working transport. |
...
> **`EMAIL_TRANSPORT=console` (2026-08-28).** Stubs mail out entirely: the message goes to stderr and *nothing is delivered* — not even to the test recipients above. It exists because the whole client-facing half of the NAR1 workflow is otherwise unreachable — `verification_sent_at` is only stamped after a successful send, so with no usable Resend key no case can be driven to Awaiting Client, Signing or Submission. It must be asked for by name (no default, never inferred from a missing key), it is **refused outright when `APP_ENV=prod`**, every send it makes is audited with `transport: "console"`, and the Client Verification screen says in as many words that nothing was delivered. Remove it the moment a real Resend key is in place — protecting clients is no longer its job, the recipient lock above does that.
>
