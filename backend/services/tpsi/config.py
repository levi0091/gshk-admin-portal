...
FORM_FEES: dict[str, tuple[bool, Decimal | None]] = {
    ...
    # source.
    #
    # THIS IS THE ON-TIME FEE ONLY, and for NAR1 it is no longer the number
    # anything charges or gates against. Late delivery attracts HK$870-HK$3,480
    # and the tier is now COMPUTED from the company's return date in
    # `services/tpsi/fees.py`, which is what `filings.preview` and the submit
    # gate use. Spec §13 put that out of scope; it was brought back in
    # (Levi, 2026-08-27) after a real submission on the CR test environment was
    # billed HK$2,610 against a HK$105 quote. Use `fees.annual_return_fee`, not
    # this table, for anything a user or a balance check will see.
