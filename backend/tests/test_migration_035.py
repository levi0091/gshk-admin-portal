...
def test_a_contains_filter_uses_its_trigram_index(conn, column):
    plan = _plan(conn, f"SELECT * FROM public.audit_log "
                       f"WHERE {column} ILIKE '%zqx%' "
                       f"ORDER BY created_at DESC LIMIT 100")
    assert f"trgm_audit_log_{column}" in plan, plan


def test_the_search_box_never_falls_back_to_a_sequential_scan(conn):
    """The whole eight-term OR, exactly as routers/audit.py builds it."""
    terms = " OR ".join(f"{c} ILIKE '%T0001138%'" for c in SEARCH_COLUMNS)
    plan = _plan(conn, f"SELECT * FROM public.audit_log WHERE {terms} "
                       f"ORDER BY created_at DESC LIMIT 100")
    assert "Seq Scan on audit_log" not in plan, plan


def test_two_enum_filters_that_match_nothing_return_immediately(conn):
    ...
    plan = _plan(conn, "SELECT * FROM public.audit_log "
                       "WHERE module = 'body_corporate' AND subject_kind = 'person' "
                       "ORDER BY created_at DESC LIMIT 100")
    assert "idx_audit_log_module_kind_created" in plan, plan
